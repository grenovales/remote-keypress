...
app.whenReady().then(() => {
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { type: "separator" },
    {
      label: "Quit Remote Keypress",
      type: "normal",
      click: () => {
        //app.quit();
        dialog
          .showMessageBox(null, {
            title: "Remote Keypress",
            message:
              "Do you want to quit Remote Keypress? \n\nSending keypress to application will to working until you open this app again.",
            buttons: ["Yes", "No"],
            cancelId: 1,
          })
          .then((result) => {
  ...
  tray.setToolTip("Remote Keypress APP");
  tray.setContextMenu(contextMenu);
  tray.on("click", () => {
    createWindow(); // Function to create and open a new window
  });

  //Database
  // Open or create a SQLite database file
  db = new sqlite3.Database("./keypress.db", (err) => {
    if (err) {
